...
python -m nanochat.dataset -n 10 # Download only 1 shard for tokenizer training
...
# No torchrun, direct python command for single GPU
# Original: torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train ...
...
# Base model evaluation - reduce eval scope or skip for speed
# Original: torchrun ... scripts.base_loss
#           torchrun ... scripts.base_eval
python -m scripts.base_loss --device_batch_size=1 --split_tokens=512 # Evaluate on very few tokens
python -m scripts.base_eval --max-per-task=16 # Evaluate on very few tasks/examples

# --- Midtraining ---
# Download synthetic data is small, can keep.
curl -L -o $NANOCHAT_BASE_DIR/identity_conversations.jsonl https://karpathy-public.s3.us-west-2.amazonaws.com/identity_conversations.jsonl

# Reduce midtraining iterations and evaluation scope
# Original: torchrun ... scripts.mid_train ...
...
python -m scripts.mid_train \
    ...
    --eval_every=-1 \
    --run=$WANDB_RUN
python -m scripts.chat_eval --source=mid --max-new-tokens=32 --max-problems=5 # Very minimal chat eval
...
python -m scripts.chat_sft \
    ...
    --run=$WANDB_RUN
python -m scripts.chat_eval --source=sft --max-new-tokens=32 --max-problems=5 # Very minimal chat eval
